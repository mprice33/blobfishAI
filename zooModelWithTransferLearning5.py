...
nasnet = nasnet.NASNetLarge(include_top=False, weights='imagenet', input_shape=input_shape)

output = nasnet.layers[-1].output
output = keras.layers.Flatten()(output)
nasnet_model = Model(nasnet.input, output)
...
train_features_nasnet = get_bottleneck_features(nasnet_model, train_imgs_scaled)
validation_features_nasnet = get_bottleneck_features(nasnet_model, validation_imgs_scaled)

print('Train Bottleneck Features:', train_features_nasnet.shape,
      '\tValidation Bottleneck Features:', validation_features_nasnet.shape)
...
input_shape = nasnet_model.output_shape[1]
...
history = model.fit(x=train_features_nasnet, y=train_labels_enc,
                    validation_data=(validation_features_nasnet, validation_labels_enc),
                    batch_size=batch_size,
                    epochs=epochs,
                    verbose=1)

f, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
t = f.suptitle('Transfer learning model 5 CNN Performance', fontsize=12)
...
model.save('Transfer_Learning_zoo_cnn_5.h5')