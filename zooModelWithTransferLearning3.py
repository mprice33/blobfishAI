...
inception_resnet_model = Model(inceptionResNetV2.input, output)
...
layers = [(layer, layer.name, layer.trainable) for layer in inception_resnet_model.layers]
...
train_features_inception_resnet = get_bottleneck_features(inception_resnet_model, train_imgs_scaled)
validation_features_inception_resnet = get_bottleneck_features(inception_resnet_model, validation_imgs_scaled)
...
input_shape = inception_resnet_model.output_shape[1]
...
history = model.fit(x=train_features_inception_resnet, y=train_labels_enc,
                    validation_data=(validation_features_inception_resnet, validation_labels_enc),
                    batch_size=batch_size,
                    epochs=epochs,
                    verbose=1)

f, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
t = f.suptitle('Transfer learning model 3 CNN Performance', fontsize=12)
...
model.save('Transfer_Learning_zoo_cnn_3.h5')